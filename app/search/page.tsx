    ...
    <>
      ...
      <div className="min-h-screen bg-[#FAF8F5] flex pt-16">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Map Section */}
          <div className="border-b-2 border-[#D4C4B0] relative">
            <div
              ref={mapRef}
              className="w-full h-[400px] bg-[#F5EFE7]"
              role="region"
            ...
            {!mapLoaded && (
              <div className="absolute inset-0 flex items-center justify-center bg-[#FAF8F5]/80">
                <div className="text-center">
                  ...
                  <p className="font-serif text-[#8B7355]">Loading map...</p>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="container mx-auto px-4 py-12">
              <div className="max-w-6xl mx-auto">
                <div className="text-center mb-12">
                  ...
                  <p className="font-serif text-lg text-[#8B7355] italic">
                ...
                {filteredPlaces.length > 0 ? (
                    ...
                    {filteredPlaces.map((place) => (
                      <div
                        ...
                        className={`transition-all ${
                          selectedPlaceId === place.placeId ? 'ring-4 ring-[#8B7355] rounded-lg' : ''
                        }`}
                      >
                        <ListingCard
                          listing={{
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="font-serif text-sm text-[#A0826D]">
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="font-serif text-xl text-[#8B7355] mb-6">
                      Your journey begins with a single search
                    </p>
                    <div className="max-w-md mx-auto space-y-4">
                      <button
                        onClick={() => handleSearch('assisted living apartments in Cleveland OH')}
                        className="w-full px-6 py-3 border-2 border-[#8B7355] text-[#8B7355] font-serif rounded-lg hover:bg-[#8B7355] hover:text-white transition-colors"
                      >
      ...
      >
        {selectedListing && (
          <div className="space-y-4">
            <div>
              <h3 className="font-medium text-[#5C4A3C] mb-2">Address</h3>
              <p className="text-[#8B7355]">{selectedListing.formattedAddress}</p>
            </div>

            {selectedListing.rating && (
              <div>
                <h3 className="font-medium text-[#5C4A3C] mb-2">Rating</h3>
                <p className="text-[#8B7355]">⭐ {selectedListing.rating.toFixed(1)} / 5.0</p>
              </div>
            )}

            {selectedListing.priceLevel && (
              <div>
                <h3 className="font-medium text-[#5C4A3C] mb-2">Estimated Price Range</h3>
                <p className="text-[#8B7355]">${selectedListing.priceLevel * 500} - ${selectedListing.priceLevel * 800}/month</p>
              </div>
            )}

            <div className="pt-4 border-t border-[#D4C4B0] flex gap-3">
              <button
                className="flex-1 px-4 py-2 bg-[#8B7355] text-[#FAF8F5] rounded-md hover:bg-[#5C4A3C] transition-colors"
              ...
              <button
                className="px-4 py-2 border border-[#8B7355] text-[#8B7355] rounded-md hover:bg-[#8B7355] hover:text-[#FAF8F5] transition-colors"
